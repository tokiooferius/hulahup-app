    ...
    <div class="max-w-6xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-8">
            <div class="relative h-48 bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center">
                <img src="{{ $canteen->logo_url ?? 'https://via.placeholder.com/600x200?text=' . urlencode($canteen->name) }}" 
                     alt="{{ $canteen->name }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        @foreach($menus as $menu)
                            <div class="bg-white rounded-xl shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden group"
                                 onclick="showMenuDetail({{ json_encode($menu) }})">
                                ...
                                <div class="p-4">
                                    ...
                                    <div class="flex gap-2">
                                        <button class="flex-1 bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-3 rounded-lg transition flex items-center justify-center gap-1 text-sm"
                                                onclick="event.stopPropagation(); showMenuDetail({{ json_encode($menu) }})">
                                        ...
                                        <button class="flex-1 bg-orange-500 hover:bg-orange-600 text-white font-semibold py-2 px-3 rounded-lg transition flex items-center justify-center gap-1 text-sm"
                                                onclick="event.stopPropagation(); addMenuToCart({{ $menu->id }}, '{{ $menu->name }}', {{ $menu->price }}, '{{ $menu->image_url }}', {{ $canteen->id }})">
        ...
        const html = `
            <div class="space-y-4">
                ...
                <div class="flex gap-3 pt-4 border-t border-slate-200">
                    ...
                    <button onclick="event.stopPropagation(); addMenuToCart(${menu.id}, '${menu.name}', ${menu.price}, '${menu.image_url}', ${menu.canteen_id}); closeMenuDetail();" 
