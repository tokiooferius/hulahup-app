...
@section('content')
<div class="min-h-screen bg-gradient-to-br from-blue-50 to-slate-50 p-8">
    <!-- Header -->
    <div class="mb-10">
        ...
        <div class="bg-white rounded-xl p-6 shadow-md border-l-4 border-orange-500">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-3xl font-bold text-orange-600">{{ $pendingOrders->count() }}</h3>
                </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow-md border-l-4 border-blue-500">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-3xl font-bold text-blue-600">{{ $processingOrders->count() }}</h3>
                </div>
                <span class="text-3xl">🔄</span>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow-md border-l-4 border-green-500">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-3xl font-bold text-green-600">{{ $completedOrders->count() }}</h3>
                </div>
                <span class="text-3xl">✅</span>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl p-6 shadow-md border-l-4 border-purple-500">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-3xl font-bold text-purple-600">{{ $totalOrders }}</h3>
                </div>
                <span class="text-3xl">📦</span>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="flex border-b">
            ...
            <button class="tab-btn flex-1 py-4 px-6 font-bold text-center border-b-4 border-transparent text-slate-600 hover:bg-slate-50 transition" data-tab="processing">
                🔄 DIPROSES ({{ $processingOrders->count() }})
            </button>
            ...
            @forelse($pendingOrders as $order)
                <div class="order-card bg-gradient-to-r from-orange-50 to-yellow-50 rounded-lg p-6 mb-4 border-l-4 border-orange-500 shadow-sm hover:shadow-md transition">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h3 class="text-lg font-bold text-[#122C4F]">{{ $order->order_number }}</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div>
                            ...
                            @foreach($items as $item)
                                <p class="text-sm font-medium text-slate-700">{{ $item['name'] }} x{{ $item['qty'] }}</p>
                            @endforeach
                        </div>
                        <div>
                            <p class="text-xs text-slate-500 font-semibold uppercase">Total</p>
                            <p class="text-lg font-bold text-orange-600">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-slate-500 font-semibold uppercase">Waktu Pesan</p>
                            <p class="text-sm font-medium text-slate-700">{{ $order->created_at->format('d M Y H:i') }}</p>
                        </div>
                    </div>

                    <div class="flex gap-3">
                        <button onclick="updateOrderStatus({{ $order->id }}, 'processing')" class="flex-1 bg-blue-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-blue-700 transition">
                            ➜ Mulai Proses
                        </button>
                        <button onclick="updateOrderStatus({{ $order->id }}, 'cancelled')" class="px-6 py-3 bg-red-100 text-red-700 rounded-lg font-bold hover:bg-red-200 transition">
                            ❌ Batal
                        </button>
                    </div>
                </div>
                ...
                <div class="order-card bg-gradient-to-r from-blue-50 to-cyan-50 rounded-lg p-6 mb-4 border-l-4 border-blue-500 shadow-sm hover:shadow-md transition">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-slate-700">{{ $order->processing_at?->format('d M Y H:i') ?? '-' }}</p>
                        </div>
                    </div>

                    <div class="flex gap-3">
                        <button onclick="updateOrderStatus({{ $order->id }}, 'completed')" class="flex-1 bg-green-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-green-700 transition">
                            ✅ Selesaikan
                        </button>
                        <button onclick="updateOrderStatus({{ $order->id }}, 'pending')" class="px-6 py-3 bg-slate-200 text-slate-700 rounded-lg font-bold hover:bg-slate-300 transition">
                            ↩️ Kembali Pending
                        </button>
                    </div>
                </div>
                ...
                <div class="order-card bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-6 mb-4 border-l-4 border-green-500 shadow-sm hover:shadow-md transition">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-slate-700">{{ $order->completed_at?->format('d M Y H:i') ?? '-' }}</p>
                        </div>
                    </div>
                </div>
            @empty
    ...
    <!-- Back Button -->
    <div class="mt-10">
        <a href="{{ route('canteen.dashboard') }}" class="inline-block bg-slate-200 text-slate-700 px-6 py-3 rounded-lg font-bold hover:bg-slate-300 transition">
